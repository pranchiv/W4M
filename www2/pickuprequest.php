...
<div class="wrapper row3">
	<div class="container low-pad-top">
        ...
        <?php
		$query = "select * from Donation where addDate >= '$dateToday' and foodStatus = 'hold'";
			...
			foreach($donateReq as $recdata)
			{
				...
				if($endstr>$currenttimestamp)
				{
					...
					?>
                        ...
                        <input type="hidden" name="restauID[]" value="<?=$recdata['donorId']?>" />
                        <tr>
                        	<td><input type="checkbox" name="reqID[]" value="<?=$recdata['id']?>" /></td>
		...
		{
			?><h2 class="h2fordriver">Currently no requests available.</h2><?php
		}
		?>
        </form>
    </div>
